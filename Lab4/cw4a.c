...
unsigned long time_mikrosec(void)
{
    struct timeval tv;
...
int main(int argc, char *argv[])
{
    ...
    unsigned long prev_wakeup = 0;
    unsigned long prev_smptime = 0;

    unsigned int fix_sleep = !!getenv("CW4A_FIX_SLEEP");
    ...
    for(i=0; i<nsmp; i++) {
        ...
        if (fix_sleep) 
        {
            unsigned long prev_processed_time = time_mikrosec() - prev_wakeup;
            if (udelsmp > prev_processed_time) usleep(udelsmp - prev_processed_time);
        }
        else
        {
            usleep(udelsmp);
